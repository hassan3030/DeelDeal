...
const Swap = () => {
  const otherId = "c49a1579-f579-49dc-a5c1-4bda0de4059c";
  // my items to display
  ...
  // Function to handle the selection of my unAvailable items
  ...
  const getOtherItems = async () => {
    const myProductsData = await getAvailableAndUnavailableProducts(otherId);
    setOtherItems(myProductsData);
  };
  ...
  useEffect(() => {
    ...
    console.log("selectedMyItems", selectedMyItems);
    console.log("selectedOtherItems", selectedOtherItems);
  }, [selectedMyItems]);
  ...
  const handleOtherItemSelect = (itemId) => {
    ...
    // Check if any allowed_categories match
  ...
  const canCreateSwap =
    selectedMyItems.length > 0 && selectedOtherItems.length > 0;

    const handleAddOffer = async () => {
      try{
 const to_user = await getUserByProductId('dde81554-19cb-475e-aa6a-82bc2c2b03f9');
    await addOffer(
      to_user.id,
      priceDifference,
      selectedMyItems,
    ...
    toast({
        ...
        description: "Successfuly created offer",
...
toast({
        title: t("faildSwap") || "Failed Swap",
        description: "Invalid swap without login. Please try again.",
        variant: "destructive",
      });
      }
...
if(userId === id){
 if (cash > 0){
  return `You pay: ${Math.abs(Math.ceil(cash))} LE`;
 }
    else if (cash < 0){
      return `You get: ${Math.abs(Math.ceil(cash))} LE` ;
    }
    else {
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {otherItems.map((product) => {
                      ...
                      const isSelected = selectedOtherItems.includes(
                        product.id
                      );
                              ...
                              <div className="flex flex-col">
                                <span className="text-sm text-muted-foreground">
                                  ...
                                  {/* {swap.cash_adjustment > 0
                                    ? `Take: ${Math.abs(
                                        Math.floor(swap.cash_adjustment)
                                      )}`
                                    : swap.cash_adjustment === 0
                                    ? `Product Have The Same Price`
                                    : `Pay: ${Math.abs(
                                        Math.floor(swap.cash_adjustment)
                                      )}`}{" "} */}
                                </span>
