...
import { Search, ShoppingCart, Bell , Menu, X, User, Moon, Sun, Settings, LogOut } from "lucide-react"
...
import { getProductSearchFilter , getOfferById, getOffersNotifications } from "@/callAPI/products"
...
export function Header() {
  ...
  const handlegetProductSearchFilter = async ()=>{
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background shadow-sm">
      ...
      <div className="container py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="relative hidden flex-1 md:block">
            ...
            <Input
              ...
              className={`${isRTL ? "pr-10" : "pl-10"} rounded-full border-2 border-primary`}
              value={filter}
              onChange={(e)=>{serFilter(e.target.value)}}
            />
          </div>
          ...
          <div className="hidden items-center gap-4 md:flex">
            {user ? (
              ...
              <>
                <Button variant="ghost" size="sm" asChild className="gap-1 text-sm hover:text-primary">
                  <Link href="/auth/login">{t("signIn")}</Link>
                </Button>
                <Button
                  size="sm"
                  asChild
                  className={cn("gap-1 text-sm bg-primary text-primary-foreground hover:bg-primary/90")}
                >
          ...
          {/* Mobile menu button */}
          <Button variant="ghost" size="icon" className="hover:text-primary md:hidden" onClick={toggleMenu}>
        ...
        {isMenuOpen && (
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="sm" className="gap-2 hover:text-black hover:bg-primary/10">
                    {/* {user?.avatar ? (
                      <img
                        src={`http://localhost:8055/assets/${user.avatar}` || "/placeholder.svg"}
                        alt={user?.first_name || t("account")}
                    ...
                    {<Avatar className="h-12 w-12">
                                            <AvatarImage src={user?.avatar } alt={user?.first_name || t("account")} />
              ...
              <Link href="/notifications" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10">
                <Bell className="h-4 w-4" />
                <span>{`${t("notifications")} ${notificationsLength} `} </span>
              </Link>
              <Link href="/cart" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10">
                <ShoppingCart className="h-4 w-4" />
                <span>{`${t("cart")} ${cartLength} `}</span>
              </Link>
              <Link href="/customerService" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10">
                <span>{t("customerService") }</span>
              </Link>

               <Link href="/" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10"  onClick={() => logout()}>
          ...
          (<div className="mt-4 border-t pt-4 md:hidden">
            <nav className="flex flex-col gap-2">
              ...
              <div className="my-2 border-t"></div>
              <Link href="/auth/login" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10">
                <User className="h-4 w-4" />
                <span>{t("signIn")}</span>
              </Link>
              <Link href="/customerService" className="flex items-center gap-2 rounded-md px-3 py-2 hover:bg-primary/10">
